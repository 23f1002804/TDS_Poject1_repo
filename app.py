...
MAX_RESULTS = 16  # Increased to get more context
load_dotenv()
...
def normalize_url_remove_query(url: str) -> str:
    url=url.split('?')[0]  # Remove ? from normal query if any
    return url
        ...
        for chunk in markdown_chunks:
            try:
                ...
                if similarity >= SIMILARITY_THRESHOLD:
                    ...
                    url = normalize_url_remove_query(url)
        ...
        if len(parts) > 1:
            ...
            for line in source_lines:
                ...
                if url_match:
                    ...
                    url = url.strip()
                    url = normalize_discourse_url(url)
